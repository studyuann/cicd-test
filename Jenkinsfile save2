pipeline {
    ...
    steps {
        sshagent(credentials: ['Deploy-Privatekey']) {
            ...
            sh "scp -o StrictHostKeyChecking=no index.html ubuntu@3.35.149.250:/home/ubuntu/"
            // 2. SSH로 접속하여 sudo 권한으로 파일을 웹 서버 경로로 복사합니다.
            // 주의: 서버의 ubuntu 계정이 'sudoers'에서 NOPASSWD 설정이 되어 있어야 합니다.
            sh "ssh -o StrictHostKeyChecking=no ubuntu@3.35.149.250 sudo cp /home/ubuntu/index.html /var/www/html/"
                 }
            }
        }
    }
